...
%global _servicedir %{_exec_prefix}/lib/systemd/system
...
%endif
Requires:       ceph-mgr = %{_epoch_prefix}%{version}-%{release}
%if 0%{?fedora} || 0%{?rhel} || 0%{?openEuler} > 7 || 0%{?suse_version}
...
export CFLAGS="$RPM_OPT_FLAGS -zcommon-page-size=2097152 -zmax-page-size=2097152 -lhugetlbfs -Wl,-hugetlbfs-align"
export CXXFLAGS="$RPM_OPT_FLAGS -zcommon-page-size=2097152 -zmax-page-size=2097152 -lhugetlbfs -Wl,-hugetlbfs-align"
export LDFLAGS="$RPM_LD_FLAGS -zcommon-page-size=2097152 -zmax-page-size=2097152 -lhugetlbfs -Wl,-hugetlbfs-align"
...
${CMAKE} .. \
    ...
    -DCMAKE_INSTALL_SYSTEMD_SERVICEDIR=%{_servicedir}\
    -DWITH_MANPAGE=ON \
...
/usr/sbin/useradd ceph -u $CEPH_USER_ID -o -r -g ceph -s /bin/bash -c "Ceph daemons" -d %{_localstatedir}/lib/ceph 2>/dev/null || :
%endif
%if 0%{?suse_version}
...
%{_libexecdir}/ceph/ceph-osd-spdk.sh
%{_libexecdir}/ceph/enable-spdk-conf.sh
%{_libexecdir}/ceph/disable-spdk-conf.sh
%{_libexecdir}/ceph/create-osd.sh
%{_libexecdir}/ceph/delete-osd.sh
%{_libexecdir}/ceph/spdk/scripts/common.sh
%{_libexecdir}/ceph/spdk/scripts/setup.sh
%{_libexecdir}/ceph/spdk/include/spdk/pci_ids.h
...
%{_unitdir}/spdk-conf.service
%{_unitdir}/spdk-conf.target
%attr(750,ceph,ceph) %dir %{_localstatedir}/lib/ceph/osd
%config(noreplace) %{_sysctldir}/90-ceph-osd.conf
%{_sysconfdir}/sudoers.d/ceph-osd-smartctl

%post osd
setcap 'CAP_DAC_OVERRIDE+eip CAP_SYS_ADMIN+eip' %{_bindir}/ceph-osd
rm -f %{_bindir}/ceph-osd.in
cp -f %{_bindir}/ceph-osd %{_bindir}/ceph-osd.in
%if 0%{?suse_version}
if [ $1 -eq 1 ] ; then
  /usr/bin/systemctl preset spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target >/dev/null 2>&1 || :
fi
%endif
%if 0%{?fedora} || 0%{?rhel} || 0%{?openEuler}
%systemd_post spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target
%endif
if [ $1 -eq 1 ] ; then
/usr/bin/systemctl start spdk-conf.target ceph-osd.target >/dev/null 2>&1 || :
fi
%if 0%{?sysctl_apply}
...
%preun osd
%if 0%{?suse_version}
%service_del_preun spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target
%endif
%if 0%{?fedora} || 0%{?rhel} || 0%{?openEuler}
%systemd_preun spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target
%endif

%postun osd
%if 0%{?suse_version}
DISABLE_RESTART_ON_UPDATE="yes"
%service_del_postun spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target
%endif
%if 0%{?fedora} || 0%{?rhel} || 0%{?openEuler}
%systemd_postun spdk-conf.service spdk-conf.target ceph-osd@\*.service ceph-volume@\*.service ceph-osd.target
%endif
if [ $1 -ge 1 ] ; then
  # Restart on upgrade, but only if "CEPH_AUTO_RESTART_ON_UPGRADE" is set to
  # "yes". In any case: if units are not running, do not touch them.
  ...
  if [ "X$CEPH_AUTO_RESTART_ON_UPGRADE" = "Xyes" ] ; then
    /usr/bin/systemctl try-restart spdk-conf.service ceph-osd@\*.service ceph-volume@\*.service > /dev/null 2>&1 || :
  fi
fi
