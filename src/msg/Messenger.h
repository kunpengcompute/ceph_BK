...
public:
  ...
  virtual ConnectionRef connect_to(
    int type, const entity_addrvec_t& dest, bool not_local_dest=false) = 0;
  ConnectionRef connect_to_mon(const entity_addrvec_t& dest, bool not_local_dest=false) {
    return connect_to(CEPH_ENTITY_TYPE_MON, dest, not_local_dest);
  }
  ConnectionRef connect_to_mds(const entity_addrvec_t& dest, bool not_local_dest=false) {
    return connect_to(CEPH_ENTITY_TYPE_MDS, dest, not_local_dest);
  }
  ConnectionRef connect_to_osd(const entity_addrvec_t& dest, bool not_local_dest=false) {
    return connect_to(CEPH_ENTITY_TYPE_OSD, dest, not_local_dest);
  }
  ConnectionRef connect_to_mgr(const entity_addrvec_t& dest, bool not_local_dest=false) {
    return connect_to(CEPH_ENTITY_TYPE_MGR, dest, not_local_dest);
  }
